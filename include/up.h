...
typedef struct up_context_struct {
    ...
    /** Grouch FSM state; > 0 are real statues, < 0 are used by the protocol machine */
    int grouchfsm;
...
/** Attach a BIO to an UP context; the context claims the BIO and
 *  will call destroy() on it in due course 
 */
int up_attach_bio(up_context_t *ctx, up_bio_t *bio);

/** Dispose of an up context and call dispose() on any 
 *  associated BIO
 */
...
/** Start the console 
 *
 * @param[in] tty_fd  The fd to our controlling tty - either 0 for stdin, or a pty you
 *                     have created.
 */
...
/** Become a console, wait until someone says it should quit, then finish it.
 *  Useful for turning your UPC program into a console at the end 
 */
